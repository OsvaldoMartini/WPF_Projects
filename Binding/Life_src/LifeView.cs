...
using Binding.GameOfLife.XBAL.Utility;

namespace Binding.GameOfLife.XBAL
{
	/// <summary>
	/// LifeView provides a visual interface for the LifeModel.
	/// </summary>
	public class LifeView : FrameworkElement	// public so Reflection/XAML works!
	{
		...
		{
			if (_model == null)
				_model = new LifeModel(this.Dimensions);
			else
		...
		{
			// We don't use the dc to draw into directly, instead we draw into child visuals to allow us to
			// draw each layer independently. Its handy to put the draw code here
			// as various dp's that are flagged with AffectRender will be handled correctly (otherwise we would have to add
